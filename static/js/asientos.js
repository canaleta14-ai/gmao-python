...
function configurarEventos() {
  // Eventos de filtros
  document
    .getElementById("filtro-numero")
    .addEventListener("input", debounce(aplicarFiltros, 300));
  document
    .getElementById("filtro-cuenta")
    .addEventListener("input", debounce(aplicarFiltros, 300));

  // Eventos del modal
  document
    .getElementById("modal-asiento")
...
function generarDatosMockAsientos() {
  ...
  for (let i = 1; i <= 25; i++) {
    ...
    asientos.push({
      ...
      concepto: `Asiento contable ${i} - ${
        ["Compra repuestos", "Pago servicios", "Regularización"][
...
function mostrarAsientos() {
  ...
  let asientosFiltrados = asientosData.filter((asiento) => {
    return (
      (!filtros.numero ||
        asiento.numero.toLowerCase().includes(filtros.numero.toLowerCase())) &&
      (!filtros.cuenta ||
        asiento.lineas.some((linea) =>
          linea.cuenta_contable.includes(filtros.cuenta)
        )) &&
  ...
  asientosPagina.forEach((asiento) => {
    ...
    fila.innerHTML = `
            ...
            <td>
                <span class="badge bg-${obtenerColorEstado(asiento.estado)}">${
      asiento.estado
    }</span>
            </td>
            <td>
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-sm btn-outline-info" onclick="verAsiento(${
                      asiento.id
                    })" title="Ver detalle">
                        <i class="bi bi-eye"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-primary" onclick="editarAsiento(${
                      asiento.id
                    })" 
                            title="Editar" ${
                    ...
                    <button class="btn btn-sm btn-outline-danger" onclick="eliminarAsiento(${
                      asiento.id
                    })" 
                            title="Eliminar" ${
                              asiento.estado === "cerrado" ? "disabled" : ""
  ...
  document.getElementById("info-paginacion").textContent = `Mostrando ${
    inicio + 1
  }-${Math.min(fin, asientosFiltrados.length)} de ${
...
function actualizarEstadisticas() {
  ...
  const totalDebe = asientosFiltrados.reduce(
    (sum, asiento) => sum + asiento.total_debe,
    0
  );
  const totalHaber = asientosFiltrados.reduce(
    (sum, asiento) => sum + asiento.total_haber,
    0
  );
...
function nuevoAsiento() {
  ...
  const proximoNumero = `AST-${String(asientosData.length + 1).padStart(
    4,
    "0"
  )}`;
  document.getElementById("asiento-numero").value = proximoNumero;

  document.getElementById("titulo-modal-asiento").textContent =
...
function verAsiento(id) {
  ...
  contenido.innerHTML = `
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <strong>Estado:</strong> <span class="badge bg-${obtenerColorEstado(
                  asiento.estado
                )}">${asiento.estado}</span>
            </div>
            <div class="col-md-6">
                <strong>Total Debe:</strong> ${formatearMoneda(
                  asiento.total_debe
                )}<br>
                <strong>Total Haber:</strong> ${formatearMoneda(
                  asiento.total_haber
                )}<br>
                <strong>Balance:</strong> <span class="balance-${
                  asiento.total_debe === asiento.total_haber ? "ok" : "error"
                }">
                    ${formatearMoneda(
                      Math.abs(asiento.total_debe - asiento.total_haber)
                    )}
                </span>
            </div>
        </div>
            ...
            <table class="table table-bordered">
                    ...
                    ${asiento.lineas
                      .map(
                        ...
                        <tr>
                            ...
                            <td class="debe-cell">${
                              linea.debe ? formatearMoneda(linea.debe) : "-"
                            }</td>
                            <td class="haber-cell">${
                              linea.haber ? formatearMoneda(linea.haber) : "-"
                            }</td>
                        </tr>
                    `
                      )
                ...
                <tfoot class="table-info">
                    <tr>
                        ...
                        <td class="debe-cell"><strong>${formatearMoneda(
                          asiento.total_debe
                        )}</strong></td>
                        <td class="haber-cell"><strong>${formatearMoneda(
                          asiento.total_haber
                        )}</strong></td>
                    </tr>
                </tfoot>
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("modal-ver-asiento")
  );
  modal.show();
}
...
function calcularTotales() {
  ...
  document.getElementById("total-debe-modal").textContent =
    formatearMoneda(totalDebe);
  document.getElementById("total-haber-modal").textContent =
    formatearMoneda(totalHaber);
...
async function guardarAsiento() {
  ...
  try {
    ...
    // Calcular totales
    asientoData.total_debe = asientoData.lineas.reduce(
      (sum, linea) => sum + linea.debe,
      0
    );
    asientoData.total_haber = asientoData.lineas.reduce(
      (sum, linea) => sum + linea.haber,
      0
    );
    ...
    bootstrap.Modal.getInstance(
      document.getElementById("modal-asiento")
    ).hide();
    mostrarAsientos();
    actualizarEstadisticas();
    generarPaginacion();
...
function mostrarVistaDetallada() {
  ...
  container.innerHTML = asientosFiltrados
    .map(
      (asiento) => `
        <div class="card mb-3">
                            ...
                            ${asiento.lineas
                              .map(
                                ...
                                <tr>
                                    ...
                                    <td class="debe-cell">${
                                      linea.debe
                                        ? formatearMoneda(linea.debe)
                                        : "-"
                                    }</td>
                                    <td class="haber-cell">${
                                      linea.haber
                                        ? formatearMoneda(linea.haber)
                                        : "-"
                                    }</td>
                                </tr>
                            `
...
function confirmarEliminarAsiento() {
  if (asientoAEliminar) {
    ...
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("modalEliminarAsiento")
    );
    if (modal) {
      modal.hide();
    }
