...
function mostrarOrdenes() {
    ...
    ordenes.forEach((orden) => {
        ...
        row.innerHTML = `
            ...
            <td><span class="badge bg-${getBadgeColor(orden.prioridad)}">${orden.prioridad
            }</span></td>
            <td><span class="badge bg-${getEstadoBadgeColor(orden.estado)}">${orden.estado
            }</span></td>
            <td class="col-tecnico">${orden.tecnico_nombre || "Sin asignar"
            }</td>
            <td>
                <div class="btn-group btn-group-sm" role="group">
                    <button type="button" class="btn btn-sm btn-outline-primary action-btn view" onclick="verOrden(${orden.id
            })" title="Ver detalles">
                        <i class="bi bi-eye"></i>
                    </button>
                    <button type="button" class="btn btn-sm btn-outline-info action-btn info" onclick="mostrarModalEstado(${orden.id
            })" title="Cambiar estado">
                        <i class="bi bi-arrow-repeat"></i>
                    </button>
                    ${(orden.estado === 'Pendiente' || orden.estado === 'Cancelada') ? 
                        `<button type="button" class="btn btn-sm btn-outline-danger action-btn delete" onclick="mostrarModalEliminarOrden(${orden.id})" title="Eliminar orden">
...
function verOrden(id) {
    ...
    if (orden) {
        ...
        document.getElementById(
            "ver-orden-prioridad"
        ).innerHTML = `<span class="badge bg-${getBadgeColor(orden.prioridad)}">${orden.prioridad
        }</span>`;
        document.getElementById(
            "ver-orden-estado"
...
function inicializarAutocompletado() {
    ...
    if (activoInput && window.AutoComplete) {
        new AutoComplete({
            ...
            localData: activos,
            displayKey: (item) =>
                `${item.nombre} - ${item.ubicacion || "Sin ubicación"} (${item.codigo
                })`,
            valueKey: "id",
    ...
    archivoDiv.innerHTML = `
        <div class="archivo-info">
            ...
            <div class="archivo-detalles">
                ...
                <small>${formatearTamaño(archivo.tamaño || archivo.tamaño)} ${archivo.descripcion ? "- " + archivo.descripcion : ""
        }</small >
    ...
    recambios.forEach((recambio) => {
        ...
        row.innerHTML = `
            ...
            <td>
                ...
                : `<input type="number" class="form-control form-control-sm" value="${recambio.cantidad_utilizada ||
                recambio.cantidad_solicitada
                }" 
                            min="0" max="${recambio.cantidad_solicitada
                }" onchange="actualizarCantidadRecambio(${recambio.id
                }, this.value)">`
            }
            </td>
            <td class="${stockClass}">${recambio.articulo?.stock_actual || 0
            }</td>
            <td>${estado}</td>
            <td>
                ${!recambio.descontado
...
// Función auxiliar para descontar recambios confirmado (v2)
async function descontarRecambiosConfirmado() {
    try {
        ...
        try {
            ...
            if (!response.ok) {
                mostrarMensaje(`Error del servidor (${response.status}): No se pudo procesar la respuesta`, "danger");
                return;
            }
            throw jsonError;
        }
        ...
        if (response.ok) {
            ...
            let mensajeError = result.error || `Error del servidor (${response.status})`;

            if (mensajeError.includes("orden completada") || mensajeError.includes("orden cancelada")) {
...
async function asignarTecnicoMasivo() {
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Seleccione el técnico para ${seleccionados.length} orden(es):</p>
    ...
    const modal = new bootstrap.Modal(document.getElementById('modalAsignarTecnicoMasivo'));
    modal.show();

    // Limpiar modal al cerrarse
    document.getElementById('modalAsignarTecnicoMasivo').addEventListener('hidden.bs.modal', function () {
        this.remove();
    });
}
...
async function confirmarAsignarTecnicoMasivo() {
    ...
    const seleccionados = seleccionMasiva.obtenerSeleccionados();
    const tecnicoNombre = tecnicos.find(t => t.id == tecnicoId)?.nombre || 'Técnico';

    // Cerrar modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('modalAsignarTecnicoMasivo'));
    modal.hide();
    ...
    if (exitosos > 0) {
        mostrarMensaje(
            `${exitosos} orden(es) asignada(s) a ${tecnicoNombre}${errores > 0 ? `. ${errores} error(es).` : ''}`,
        ...
        <div class="modal fade" id="modalPrioridadMasiva" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Seleccione la nueva prioridad para ${seleccionados.length} orden(es):</p>
...
async function confirmarCambioPrioridadMasivaOrdenes() {
    ...
    if (exitosos > 0) {
        mostrarMensaje(
            `${exitosos} orden(es) actualizada(s) a prioridad "${nuevaPrioridad}"${errores > 0 ? `. ${errores} error(es).` : ''}`,
            errores > 0 ? 'warning' : 'success'
        );
        cargarOrdenes(currentPage);
        actualizarEstadisticas();
...
function exportarSeleccionados() {
    ...
    link.setAttribute('download', `ordenes_seleccionadas_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    mostrarMensaje(`${seleccionados.length} orden(es) exportada(s) exitosamente`, 'success');
}
...
function mostrarModalEliminarOrden(ordenId) {
    ...
    document.getElementById('numero-orden-eliminar').textContent = orden.numero_orden;
