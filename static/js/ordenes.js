...
async function cargarOrdenes(page = 1, perPageParam = null, filtros = {}) {
  try {
    ...
    mostrarMensaje(
      "Error al cargar las órdenes. Verifica tu conexión.",
      "danger"
    );
    ...
    if (tbody) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="text-center">Error al cargar las órdenes</td></tr>';
    }
  }
}
...
function confirmarEliminarRecambio() {
  if (recambioAEliminar) {
    ...
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("modalEliminarRecambio")
    );
    if (modal) {
      modal.hide();
    }
