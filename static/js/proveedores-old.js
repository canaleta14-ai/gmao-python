...
function mostrarProveedores(proveedoresAMostrar) {
  ...
  proveedoresAMostrar.forEach((proveedor) => {
    ...
    fila.innerHTML = `
            ...
            <td>${
              proveedor.telefono || '<span class="text-muted">—</span>'
            }</td>
            ...
            <td>
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-secondary action-btn edit" onclick="editarProveedor(${
                      proveedor.id
                    })" title="Editar">
                        <i class="bi bi-pencil"></i>
                    </button>
                    <button class="btn btn-outline-danger action-btn delete" onclick="eliminarProveedor(${
                      proveedor.id
...
function eliminarProveedor(id, nombre) {
  ...
  document
    .getElementById("modalEliminarProveedor")
    .setAttribute("data-proveedor-id", id);
...
function inicializarEventos() {
  ...
  document
    ...
    .addEventListener("change", filtrarProveedores);

  // Validación del NIF al escribir
  document
    .getElementById("proveedor-nif")
...
function inicializarAutocompletado() {
  ...
  if (filtroBuscar) {
    new AutoComplete({
      ...
      searchKey: "q",
      displayKey: (item) =>
        `${item.nombre} (${item.nif}) - ${item.contacto || "Sin contacto"}`,
      valueKey: "nombre",
  ...
  if (proveedorNombre) {
    new AutoComplete({
      ...
      onSelect: (item) => {
        ...
        if (item.contacto)
          document.getElementById("proveedor-contacto").value = item.contacto;
        if (item.direccion)
          document.getElementById("proveedor-direccion").value = item.direccion;
      },
    });
  }
  ...
  if (proveedorContacto) {
    ...
    const contactosUnicos = [
      ...new Set(
        proveedores
          .filter(
            (proveedor) => proveedor.contacto && proveedor.contacto.trim()
          )
