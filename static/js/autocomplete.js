...
class AutoComplete {
    ...
    createDropdown() {
        ...
        this.dropdown.className =
            "autocomplete-dropdown position-absolute w-100 bg-white border rounded shadow-sm";
    ...
    handleFocus(e) {
        ...
        if (this.input.value.length >= this.minChars) {
            console.log(
                "🔍 Disparando búsqueda por focus con:",
                `"${this.input.value}"`
            );
    ...
    renderResults() {
        ...
        this.currentResults.forEach((item, index) => {
            ...
            div.className =
                "autocomplete-item px-3 py-2 cursor-pointer hover-bg-light";
    ...
    validateInput() {
        if (!this.allowFreeText && this.input.value) {
            ...
            const validItem = this.currentResults.find(
                (item) => this.getDisplayValue(item).toLowerCase() === currentValue
            );
...
const AutoCompletePresets = {
    ...
    proveedores: {
        ...
        customFilter: (item, query) => {
            ...
            return (
                (item.nombre && item.nombre.toLowerCase().includes(q)) ||
                (item.nif && item.nif.toLowerCase().includes(q)) ||
