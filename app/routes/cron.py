...
@cron_bp.route("/verificar-planes-temp", methods=["GET"])
@csrf.exempt
def verificar_planes_temp():
    """
    Endpoint temporal para verificar planes sin protección de Cloud Scheduler.
    """
    try:
        logger.info("🔍 Iniciando verificación temporal de planes en producción")
        ...
        for plan in planes:
            ...
            # Verificar si existe el plan ID 3
            if plan.id == 3:
                plan_id_3_existe = True
        
        response = {
            "success": True,
            "total_planes": len(planes_list),
            "planes_auto_test": len(planes_auto_test),
            "planes_activos": len(planes_activos),
            "plan_id_3_existe": plan_id_3_existe,
            "planes": planes_list,
            "detalles_auto_test": planes_auto_test,
            "detalles_activos": planes_activos
        }
        
        logger.info(f"✅ Verificación completada: {len(planes_list)} planes encontrados")
