                    ...
                    ordenes_creadas.append(
                        {
                            ...
                            "activo": (
                                Activo.query.get(_get_val(plan, "activo_id")).nombre
                                if _get_val(plan, "activo_id")
                                else "N/A"
                            ),
...
def aplicar_parches_db():
    ...
    try:
        ...
        # DESHABILITADO: Preparación automática de plan candidato que creaba órdenes en cada deploy
        # Esta funcionalidad causaba la generación no deseada de órdenes en cada deployment
        ...
        try:
            ...
            if plan_candidato_id:
                # COMENTADO: No modificar automáticamente los planes en cada deploy
                # db.session.execute(
                #     text(
                ...
                # db.session.commit()
                logger.info(
                    f"PLAN ENCONTRADO pero NO modificado para evitar órdenes automáticas: id={plan_candidato_id}"
                )
            else:
                logger.info(
                    "No se encontró plan activo (no se realizarán modificaciones automáticas)"
                )
